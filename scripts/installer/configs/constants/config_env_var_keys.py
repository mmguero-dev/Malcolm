...
KEY_ENV_ARKIME_MANAGE_PCAP_FILES = (
    "MANAGE_PCAP_FILES"  # Whether or not Arkime is allowed to delete uploaded/captured PCAP
)
...
KEY_ENV_ARKIME_LIVE_COMP_TYPE = "ARKIME_COMPRESSION_TYPE"  # compression method for live Arkime capture
KEY_ENV_ARKIME_LIVE_COMP_LEVEL = "ARKIME_COMPRESSION_LEVEL"  # compression level for live Arkime capture
...
KEY_ENV_ARKIME_WISE_URL = (
    "ARKIME_WISE_SERVICE_URL"  # Used when for the Arkime WISE plugin if Arkime is performing live capture
)

KEY_ENV_ARKIME_AUTO_ANALYZE_PCAP_FILES = (
    "ARKIME_AUTO_ANALYZE_PCAP_FILES"  # automatic uploaded pcap analysis with Arkime
)
KEY_ENV_ARKIME_INDEX_MANAGEMENT_ENABLED = "INDEX_MANAGEMENT_ENABLED"  # Should Arkime use an ILM policy?
...
KEY_ENV_ARKIME_INDEX_MANAGEMENT_HISTORY_RETENTION_WEEKS = (
    "INDEX_MANAGEMENT_HISTORY_RETENTION_WEEKS"  # Number of weeks of history to keep, default 13
)
KEY_ENV_ARKIME_INDEX_MANAGEMENT_SEGMENTS = (
    "INDEX_MANAGEMENT_SEGMENTS"  # Number of segments to optimize sessions to in the ILM policy, default 1
)

KEY_ENV_CONTAINER_RUNTIME_KEY = CONTAINER_RUNTIME_KEY

KEY_ENV_FILEBEAT_TCP_LISTEN = "FILEBEAT_TCP_LISTEN"  # expose a filebeat TCP input listener
KEY_ENV_FILEBEAT_TCP_LOG_FORMAT = (
    "FILEBEAT_TCP_LOG_FORMAT"  # log format expected for events sent to the filebeat TCP input listener
)
KEY_ENV_FILEBEAT_TCP_PARSE_SOURCE_FIELD = (
    "FILEBEAT_TCP_PARSE_SOURCE_FIELD"  # source field name to parse for events sent to the filebeat TCP input listener
)
...
KEY_ENV_FILEBEAT_TCP_PARSE_DROP_FIELD = (
    "FILEBEAT_TCP_PARSE_DROP_FIELD"  # field to drop in events sent to the filebeat TCP input listener
)
...
KEY_ENV_FILEBEAT_WATCHER_POLLING = "FILEBEAT_WATCHER_POLLING"  # Use polling for file watching vs. native
...
KEY_ENV_NGINX_SSL = "NGINX_SSL"  # HTTPS (nginxSSL=True) vs unencrypted HTTP (nginxSSL=False)

# false values set "off" parameters for IPv4/IPv6 for NGINX resolver
KEY_ENV_NGINX_RESOLVER_IPV4 = "NGINX_RESOLVER_IPV4"
KEY_ENV_NGINX_RESOLVER_IPV6 = "NGINX_RESOLVER_IPV6"

KEY_ENV_OPENSEARCH_INDEX_PRUNE_THRESHOLD = "OPENSEARCH_INDEX_SIZE_PRUNE_LIMIT"  # delete based on index pattern size
KEY_ENV_OPENSEARCH_INDEX_SIZE_PRUNE_NAME_SORT = (
    "OPENSEARCH_INDEX_SIZE_PRUNE_NAME_SORT"  # delete based on index pattern size (sorted by name vs. creation time)
)
KEY_ENV_OPENSEARCH_PRIMARY = "OPENSEARCH_PRIMARY"  # OpenSearch primary instance is local vs. remote
KEY_ENV_OPENSEARCH_URL = "OPENSEARCH_URL"  # OpenSearch primary instance URL
KEY_ENV_OPENSEARCH_SSL_CERTIFICATE_VERIFICATION = (
    "OPENSEARCH_SSL_CERTIFICATE_VERIFICATION"  # OpenSearch primary instance needs SSL verification
)
KEY_ENV_OPENSEARCH_SECONDARY_URL = "OPENSEARCH_SECONDARY_URL"  # OpenSearch secondary instance URL
KEY_ENV_OPENSEARCH_SECONDARY_SSL_CERTIFICATE_VERIFICATION = (
    "OPENSEARCH_SECONDARY_SSL_CERTIFICATE_VERIFICATION"  # OpenSearch secondary instance needs SSL verification
)
KEY_ENV_OPENSEARCH_SECONDARY = "OPENSEARCH_SECONDARY"  # OpenSearch secondary remote instance is enabled
...
KEY_ENV_OPENSEARCH_DASHBOARDS_DARKMODE = "DASHBOARDS_DARKMODE"  # turn on dark mode, or not
...
KEY_ENV_SURICATA_ROTATED_PCAP = (
    "SURICATA_ROTATED_PCAP"  # rotated captured PCAP analysis with Suricata (not live capture)
)
...
KEY_ENV_ZEEK_INTEL_REFRESH_CRON_EXPRESSION = (
    "ZEEK_INTEL_REFRESH_CRON_EXPRESSION"  # Cron expression for scheduled pulls from threat intelligence feeds
)
KEY_ENV_ZEEK_INTEL_FEED_SINCE = "ZEEK_INTEL_FEED_SINCE"  # Threat indicator "since" period
