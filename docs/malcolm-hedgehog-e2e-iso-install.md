...
│   │   ├── 52. Preserved Files HTTP Server (current: Yes)
│   │   │   ├── 53. Downloaded Preserved File Password (current: ********)
│   │   │   └── 54. Zip Downloads (current: Yes)
│   │   ├── 55. Scan with Strelka (current: Yes)
│   │   └── 56. Update Scan Rules (current: Yes)
...
│       └── 61. Threat Indicator "Since" Period (current: 24 hours ago)
│       ├── 62. Use Intel on Live Traffic (current: Yes)
│       └── 63. Use Intel on Uploaded PCAP (current: Yes)
├── 64. Enrich with Reverse DNS Lookups (current: Yes)
├── 65. Enrich with Manufacturer (OUI) Lookups (current: Yes)
├── 66. Enrich with Frequency Scoring (current: Yes)
├── 67. NetBox Mode (current: Local)
│   ├── 68. Auto-Create Subnet Prefixes (current: Yes)
│   ├── 69. Auto-Populate NetBox Inventory (current: Yes)
│   ├── 70. NetBox Enrichment (current: Yes)
│   ├── 71. NetBox IP Autopopulation Filter (current: empty)
│   └── 72. NetBox Site Name (current: Malcolm)
├── 73. Expose Malcolm Service Ports (current: No)
├── 74. Network Traffic Node Name (current: host)
└── 75. Capture Live Network Traffic (current: Yes)
    ├── 76. Analyze Live Traffic with Suricata (current: Yes)
    ├── 77. Analyze Live Traffic with Zeek (current: Yes)
    ├── 78. Capture Filter (current: empty)
    ├── 79. Capture Interface(s) (current: eth0)
    ├── 80. Capture Live Traffic with netsniff-ng (current: Yes)
    ├── 81. Capture Live Traffic with tcpdump (current: No)
    ├── 82. Gather Traffic Capture Statistics (current: Yes)
    └── 83. Optimize Interface Settings for Capture (current: Yes)
...
* **(Re)generate internal passwords for Valkey**
    - Malcolm uses Valkey to store session information for several components, including NetBox. Users should answer **Y** to this question the first time authentication is configured.
